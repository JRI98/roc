        ...
        return switch (pat_data.data) {
            ...
            .record => |fields| blk: {
                ...
                for (0..destructs.len) |index| {
                    const field = GuardedList.at(destructs, index);
                    if (self.patternCanMiss(field.pattern)) break :blk true;
                }
                break :blk false;
            },
            .tuple => |items| blk: {
                const patterns = self.solved.lifted.patSpan(items);
                for (0..patterns.len) |index| {
                    const item = GuardedList.at(patterns, index);
