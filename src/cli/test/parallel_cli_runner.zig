...
const subcommand_cases = [_]CliCase{
    ...
    .{ .id = 0, .suite = .subcommands, .name = "roc build returns exit code 2 for warnings (dev)", .backend = .dev, .body = .{ .custom = .build_warning_dev } },
...
fn runCustomCase(
    ...
    const result: ?TestResult = switch (custom) {
        ...
        .build_warning_interpreter => customBuildWarning(io, allocator, &env, &timer, timeout_ms, .interpreter),
        .build_warning_dev => customBuildWarning(io, allocator, &env, &timer, timeout_ms, .dev),
